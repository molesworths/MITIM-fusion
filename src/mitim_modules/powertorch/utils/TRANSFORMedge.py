...
def gacode_to_powerstate(self, rho_vec=None):
    ...
    for key in cases_to_parameterize:
        ...
        x_src = np.asarray(input_gacode.derived["roa"], dtype=float)
        y_src = np.asarray(quant, dtype=float)
        x_ref = np.asarray(self.plasma["roa"].cpu().numpy(), dtype=float)

        mult = np.asarray(key[3], dtype=float)
        if mult.ndim > 0:
            mult = np.interp(x_src, x_ref, mult)
        y_scaled = y_src * mult

        if key[4]:
            y_safe = np.clip(y_scaled, 1e-30, None)
            grad_src = -np.gradient(np.log(y_safe), x_src)
        else:
            grad_src = np.gradient(y_scaled, x_src)

        grad_ref = np.interp(x_ref, x_src, grad_src)
        aLy_coarse = np.column_stack((x_ref, grad_ref))

        self.plasma[f"aL{key[0]}"] = torch.tensor(aLy_coarse[:, 1]).to(self.dfT)
...
def to_gacode(
    ...
    if postprocess_input_gacode is None:
...
def powerstate_to_gacode(
    ...
    Notes:
        ...
        - Edge powerstate now stores a single high-resolution plasma grid, so all
          exported quantities are mapped from that grid back to the original
          input.gacode rho grid before insertion.
    ...
    rho_gacode = self.profiles.profiles["rho(-)"]
    rho_powerstate = self.plasma["rho"][position_in_powerstate_batch, :].cpu().numpy()

    def _interp_ps_to_gacode(y_ps):
        # Profiles live on the original input.gacode grid, while plasma values
        # are stored on the powerstate grid; map back before insertion.
        return np.interp(rho_gacode, rho_powerstate, y_ps)

    def _export_custom_profiles_from_plasma():
        # These are edge-specific diagnostics/auxiliary fields. They do not fit
        # standard GACODE species semantics, so serialize them as additional
        # profile blocks that MITIM can round-trip.
        if "n0" in self.plasma:
            profiles.profiles["n0(10^19/m^3)"] = _interp_ps_to_gacode(
                self.plasma["n0"][position_in_powerstate_batch, :].cpu().numpy()
            )

        if "nz_all" in self.plasma:
            nz_ps = self.plasma["nz_all"][position_in_powerstate_batch, :, :].cpu().numpy()
            nz_gacode = np.column_stack([
                _interp_ps_to_gacode(nz_ps[:, charge_state])
                for charge_state in range(nz_ps.shape[1])
            ])
            profiles.profiles["nZ_cs(10^19/m^3)"] = nz_gacode

        if 'w0' in self.plasma:
            w0_gacode = _interp_ps_to_gacode(
                self.plasma["w0"][position_in_powerstate_batch, :].cpu().numpy()
            )
    ...
    for key in quantities:
        if key[0] in self.predicted_channels or (key[0] == "nZ" and _cs_active):
            ...
            y_new = _interp_ps_to_gacode(y_ps)
    ...
    _export_custom_profiles_from_plasma()
    ...
    if 'qpar' in self.target_options["options"]["targets_evolve"]:
        conversions['qpar_wall'] = "qpar_wall(1/m^3/s)"

    for ikey in conversions:
        if getattr(self.plasma, ikey, None) is None:
            continue
        y_new = _interp_ps_to_gacode(
            self.plasma[ikey][position_in_powerstate_batch, :].cpu().numpy()
        )
        if conversions[ikey] in profiles.profiles:
            profiles.profiles[conversions[ikey]] = y_new
        else:
...
def defineIons(self, input_gacode, rho_vec, dfT):
    ...
    data_df = pd.read_csv(
        __mitimroot__ / "src" / "mitim_modules" / "powertorch" / "physics_models" / "radiation_chebyshev.csv"
    )
