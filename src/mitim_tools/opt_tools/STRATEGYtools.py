...
def _scalarized_objective_with_X(optimization_object, Y, X=None):
    ...
    vector ``X`` when the object's signature accepts it. Backward compatible with
    the standard Y-only signature.
    """
    import inspect
    ...
    if accepts_X and X is not None:
        ...
        # counts; if X has fewer rows than Y we cannot align, so drop the X path.
...
def build_monotonicity_pairs(dv_names):
    ...
    Pairs are formed between consecutive knot DVs of the same channel. Interior
    DV names follow either ``{channel}_aLy{pos}`` / ``{channel}_d{pos}`` (current
    parameterizers) or the legacy ``aL{channel}_{pos}`` form; all are recognized.
    ``dv_names`` is the ordered list of DV names (e.g. ``list(self.bounds.keys())``),
    and the returned indices are positions into it.

    Single source of truth for the optional BO linear monotonicity constraints.
    """
    patterns = [
        ...
        for self.currentIteration in range(self.numIterations+1):
            ...
            if not self.cold_start:
                ...
                if current_step is None:
                    ...
                    print("\t* Are you aware of the consequences of continuing?",typeMsg="q" if self.askQuestions else "qa")
            ...
            # If the optimizer could not propose any genuinely-new point (its
            # optimum coincides with an already-evaluated point within tolerance,
            # surfaced as an empty x_next when stop_if_no_new_points is enabled),
            # treat this as convergence and stop before evaluating a duplicate.
            if self.x_next is not None and self.x_next.shape[0] == 0:
                print("- Optimizer proposed no new point (converged: optimum coincides with an evaluated point). Stopping.", typeMsg="i")
                self.hard_finish = True

            # ~~~~~~~~ Store class now with the next points found (after optimization)
            # Checkpoint every completed iteration (not only on cold_start) so a run
            # killed mid-loop stays readable/plottable: optimization_results.out and
            # optimization_extra.pkl are already flushed incrementally, and this keeps
            # optimization_object.pkl in step with them.
            if self.storeClass:
                self.save()
